...
$getUserInfo = $db->prepare("SELECT email FROM users WHERE user_id = :user_id LIMIT 1");
...
<div class="content container admin">
    <a href="insert.php" class="btn btn-success btn-lg "><span class="fas fa-plus"></span>Add an article</a>
    <div class="row border border-secondary table-responsive">
        <h1>
            List of articles
        </h1>
        <table class="table table-bordered">
            ...
            $statement = $db->query('SELECT articles.article_id, articles.article_title, articles.article_content, 
articles.article_img FROM articles ORDER BY article_id DESC ');
            while($item = $statement->fetch())
            {
                $dbImage = $item['article_img'];
                echo '<tr>';
                echo '<td>'.$item['article_title'].'</td>';
                echo '<td>'.substr($item['article_content'],0,150).'</td>';
                echo '<td>'.'<img height="82" width="152"class="img-fluid img-thumbnail" src="../images/'.$dbImage.'">'.'</td>';
                echo '<td>';
                echo '<a class="btn btn-secondary" href="view.php?id='.$item['article_id'].'"><span class="far fa-eye"></span>  Details</a>';
                echo '<a class="btn btn-primary" href="update.php?id='.$item['article_id'].'"><span class="far fa-edit"></span>  Edit</a>';
                echo '<a class="btn btn-danger" href="delete.php?id='.$item['article_id'].'"><span class="fas fa-minus-circle"></span>  Delete</a>';
                echo '</td>';
                echo'</tr>';
            }
            Database::disconnect();
