...
if(!empty($_POST))
{
    ...
    $db = Database::connect();
    $statement = $db->prepare("SELECT article_img FROM articles WHERE article_id = ?");
...
if(!empty($_POST))
{
    $id= checkInput($_POST['id']);
    $db = Database::connect();
    $statement = $db->prepare("DELETE FROM articles WHERE article_id = ?");
    $statement->execute(array($id));
    Database::disconnect();
...
<div class="container admin">
    <div class="row col-md-12">
        <h1 class="col-md-4"><strong>Delete an article</strong></h1>
        <br>
        <form class="form col-md-7" action="delete.php" role="form" method="post">
            <input type="hidden" name="id" value="<?php echo $id;?>"/>
            <p class="alert alert-warning">Are you sure you want to delete this article?</p>
            <div class="form-actions">
                <button type="submit" class="btn btn-warning">Yes</button>
                <a class="btn btn-primary" href="index.php"></span> No</a>
            </div>
        </form>
    </div>
</div>
